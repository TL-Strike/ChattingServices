...
class Chat extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      ...
      // Yêu cầu lịch sử tin nhắn từ server sau khi tham gia
      this.socket.emit('request-message-history', { username: this.props.username });
  ...
  registerSocketListeners = () => {
    ...
    this.socket.on('private-message-history', (messages) => {
      this.setState((prevState) => {
        const newMessages = { ...prevState.messages };
        messages.forEach(msg => {
          const recipient = msg.from === this.props.username ? msg.to : msg.from;
          if (!newMessages[recipient]) {
            newMessages[recipient] = [];
          }
          newMessages[recipient].push({ from: msg.from, message: msg.message, file: msg.file, type: 'private' });
        });
        return { messages: newMessages };
      }, () => this.scrollToBottom());
    });

    this.socket.on('group-message-history', (messages) => {
      this.setState((prevState) => {
        const newMessages = { ...prevState.messages };
        messages.forEach(msg => {
          const groupId = msg.groupId;
          // Chỉ thêm tin nhắn nếu groupId chưa có trong messages
          if (!newMessages[groupId]) {
            newMessages[groupId] = [];
          }
          // Kiểm tra xem tin nhắn đã tồn tại chưa để tránh trùng lặp
          const messageExists = newMessages[groupId].some(existingMsg =>
            existingMsg.message === msg.message &&
            existingMsg.from === msg.from &&
            existingMsg.type === msg.type &&
            JSON.stringify(existingMsg.file) === JSON.stringify(msg.file)
          );
          if (!messageExists) {
            newMessages[groupId].push({
              from: msg.from,
              message: msg.message,
              file: msg.file,
              type: msg.type || 'group',
